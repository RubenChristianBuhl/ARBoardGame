...
//  Copyright © 2016-2017 Fraunhofer IGD. All rights reserved.
//
// Version 0.3 (BETA)
//
...
/**
 * \defgroup vlOBJCSDK vlSDK Objective-C API
...
FOUNDATION_EXPORT @interface vlTrackingIssue:NSObject {
@public
    NSString *info;
    int code;
    int level;
...
FOUNDATION_EXPORT @interface vlTrackingIssues:NSObject {
@public
 ...
 */
FOUNDATION_EXPORT @protocol vlFrameListenerInterface <NSObject>
 ...
 * It is recommended using the onTrackerInitializedWithWarningIssues:andErrors: function for more precise informations about the state of the tracking pipe.
 ...
 * @param warning Optional warnings may occured during initialization.
 * @param errors Optional errors may occured during initialization.
 */
-(void)onTrackerInitializedWithWarningIssues:(vlTrackingIssues* _Nullable)warnings andErrors:(vlTrackingIssues*  _Nullable)errors;
 ...
 * Defined issues should be gathered by using onTrackerInitializedWithWarningIssues:andErrors:.
 ...
 * @param info A Dictionary holding informations about the tracking
 *
 ...
 *
 */
-(void)onTrackingInformation:(NSDictionary* _Nonnull)info;
 ...
 */
FOUNDATION_EXPORT @interface vlSDK : NSObject {
}
 ...
 */
-(id _Nonnull) initTrackerWithURI:(NSString* _Nonnull)uri andDelegate:(id _Nullable)delegate;
 ...
 * @returns a vlSDK object id
 */
-(id _Nonnull) initTrackerWithURI:(NSString* _Nonnull)uri andDelegate:(_Nullable id)delegate withOptions:(NSDictionary * _Nullable)options;
...
/** @brief (iOS only) Set the device orientation
 ...
 * @param The loglevel can be a number from 0-5, while 0 means LOG and 5 is mostly debug.
 */
 ...
 * @param debugImageName Pass NULL in order to disable the image. Otherwise you might pass: LineTrackerDebugImage0
 ...
 * This only works, when the actual pipe is a camera calibration.
 * @param command An NSString holding uri to write to.
 */
 ...
 *
 * If an empty string is passed, the uri will be local_storage_dir:InitData_XXXXX.binz, while XXXX depicts a timestamp.
 ...
 *
 * If you pass a full filename, with .binz at the end. You can explicitly save the init data under this filename.
 *
 * @returns Nothing.
 */
